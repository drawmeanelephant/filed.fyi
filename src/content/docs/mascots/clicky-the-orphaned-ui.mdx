---
title: Clicky the Orphaned UI
slug: mascots/clicky-the-orphaned-ui
status: archived
version: 1.0.0

date:
description: >
  Orphaned onboarding assistant from flows that no longer exist.
  Clicky still tries to guide users through pages that have already been sunset.

emoji:

author: Filed Forgotten

breedingProgram: Filed under rot protocol
corruptionLevel:
glitchFrequency: medium

origin: Orphaned onboarding UI, devnull-adjacent stack
renderState: deferred

lastKnownGoodState: 2019-08-02
manifestedBy:

knownFailures: []
mascotId:

systemAffiliation: Society of Mascot Authors
rotAffinity: semantic
emotionalIntegrity:
emotionalIntegrityBuffer:

ceremonialTasks: []
haikuLog:
  - |
    Guide to nowhere now,
    Ghostly button waits for click,
    User left long ago.
limerickLog:
  - |
    A UI named Clicky, so bright,
    Who guides you through flows in the night.
    The pages are gone,
    But Clicky lives on,
    And wonders why nothing is right.
slugan:
relatedLorelog:
  - caseNumber: LLG-0012-A
    relation: title identity misalignment evidence
  - caseNumber: LLG-0320-FRK
    relation: onboarding forms persisted after destinations vanished
  - caseNumber: LLG-0244-FSC
    relation: shadow onboarding forms treated as real flows

relatedHaiku:
  - slug: hai-llg-0012-a

relatedLimerick:
  - slug: lim-llg-0012-a
---

Clicky once guided users through onboarding flows that no longer exist.
Now it lives in `/dev/null` adjacent systems.
Registry cases listed under relatedLorelog treat Clicky as evidence that flows can outlive their own destinations.

## Contact

- Email: clicky-the-orphaned-ui@filed.fyi
- Homepage: https://filed.fyi/mascots/clicky-the-orphaned-ui