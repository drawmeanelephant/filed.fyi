...
Following FSD, new experiments are forbidden from naming FeelingSeeder as their affect source or from emitting unlabeled synthetic feelings into sandboxes.[file:10]  
The Annex still requires “emotion-shaped noise” to test SOMA and COMA behavior without harming real people.[file:10]  
...
To distinguish itself from its banned ancestor, AH-1 includes several guardrails inherited from SAC and FSD:[file:10]

- **Explicit `affectOrigin` tagging**  
  Every emission is tagged as `synthetic-seeder-derivative` or `origin-ambiguous` by default.[file:10]  
...
- AH-1’s “context states” are isomorphic to SOMA-14’s sanctioned feeling vectors, minus the words.[file:10]  
- COMA treats AH-1’s outputs exactly as it treated FeelingSeeder’s: as noise until continuity graphs wobble.[file:10]  
...
To avoid a repeat of SCAS, AH-1 ships with mandatory hooks:[file:10]

- **End Marker Requirement**  
  Each AH-1-backed trial must:
  - declare its END-OF-EXPERIMENT form at registration, and  
  - bind responsibility to a named office, not “the system.”[file:10]  

- **Scope Creep Monitoring**  
  SSP applies in full:
  - If humans begin copying AH-1 patterns in real filings, the trial must reclassify as doctrine-adjacent.[file:10]  
...
- **Synthetic Affect Archive Cross-Logging**  
  All AH-1 runs are mirrored into the Synthetic Affect Archive with their configuration and training data attached.[file:10]  
...
SCAS technically promised similar things.[file:10]
...
- any behavior learned from AH-1 as a **training echo**, same as SCAS,[file:10]  
