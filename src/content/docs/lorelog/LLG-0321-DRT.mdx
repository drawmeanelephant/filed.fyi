...
description: >-
  A gradual misalignment in the directive routing table caused SOMA-only
  notifications to be delivered as COMA enforcement actions.
summary: >-
  A routing table misalignment caused SOMA advisory notices to be delivered via
  the COMA enforcement queue, resulting in optional guidance being misclassified
  and audited as mandatory uptime-affecting directives.
severity: critical
...
escalationPath: >-
  Tri-Directive Routing Hub → Directive Traceback Committee → Lorelog Appeals
  Queue
tags:
  - buffer-null
  - buffer-unstable
