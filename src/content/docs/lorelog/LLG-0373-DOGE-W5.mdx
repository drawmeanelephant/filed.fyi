...
summary: >-
  Report on a Window 5 routing failure where appeals involving local-anchor
  claims and platform-delivery counterfactuals endlessly circulated between
  desks due to contested jurisdictional ownership.
description: >
  Routing failure at Window 5 in which appeals requiring local-anchor review
  were repeatedly redirected between DOGE, RAGE analysis, and the Genuine
  Experience Tribunal without ownership of the underlying counterfactual
  question.
severity: warning
disposition: filed
...
tags:
  ...
  - routing-failure
notes: >
  The queue did not fail because too many appeals arrived. It failed because
  nobody wanted to own the counterfactual.
