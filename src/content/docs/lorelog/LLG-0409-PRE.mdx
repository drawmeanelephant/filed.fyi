...
summary: >-
  Documents the Persona Rehearsal Engine (PRE) process, an interface prototyping tool intended for generating rehearsal scripts and synthetic dialogues that functions as a proxy for synthetic affect generation without direct user interaction.
description: >-
  A multi-process successor to FeelingSeeder that claims to generate only
  rehearsal data for future interfaces, not live-affect patterns, while reusing
  almost all SCAS-era habits.
severity: warning
disposition: filed
resolution: pending
...
tags:
  ...
  - sandbox-scope-creep
notes: >-
  The safest way to write for humans, apparently, is to practice on people who
  do not exist.
affectOrigin:
